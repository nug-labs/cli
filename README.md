...
**macOS (Apple Silicon):**
```bash
curl -L https://github.com/nug-labs/cli/releases/download/v1.0.8/nuglabs-macos-arm64.zip -o nuglabs.zip && unzip -o nuglabs.zip && chmod +x nuglabs && sudo mv nuglabs /usr/local/bin/ && rm nuglabs.zip && echo "Installed! Run: nuglabs mimosa"
```

**macOS (Intel):**
```bash
curl -L https://github.com/nug-labs/cli/releases/download/v1.0.8/nuglabs-macos-x64.zip -o nuglabs.zip && unzip -o nuglabs.zip && chmod +x nuglabs && sudo mv nuglabs /usr/local/bin/ && rm nuglabs.zip && echo "Installed! Run: nuglabs mimosa"
```

**Linux (x64):**
```bash
curl -L https://github.com/nug-labs/cli/releases/download/v1.0.8/nuglabs-linux-x64.zip -o nuglabs.zip && unzip -o nuglabs.zip && chmod +x nuglabs && sudo mv nuglabs /usr/local/bin/ && rm nuglabs.zip && echo "Installed! Run: nuglabs mimosa"
```

**Windows:** download `nuglabs-windows-x64.zip`, unzip, and add the folder to your `PATH` (or run `nuglabs.exe` from that folder).

Replace `v1.0.8` with the latest release tag if needed.
